...
def shard_model(
    ...
    *,
    ...
    for stage in manifest.stages:
        ...
        def flush(stage_dir: Path = stage_dir) -> None:
            nonlocal part_index, pending, pending_bytes
            if not pending:
                return
            destination = stage_dir / f"weights-{part_index:05d}.safetensors"
            ...
            with safe_open(
                ...
                for name in sorted(names):
                    ...
                    if pending_bytes >= maximum_output_file_bytes:
                        flush()
        flush()
        (stage_dir / "tensors.json").write_text(
            json.dumps(
                {
                    "stage_id": stage.stage_id,
                    "logical_weight_bytes": stage.required_memory_bytes,
                    "tensor_names": stage.tensor_names,
                },
                indent=2,
                sort_keys=True,
            )
