...
class Measurements:
    ...
    """
    def __init__(self, runs, serial_idle=None):
        self.serial_idle_source = serial_idle
        self.serial_idle = {name:[self.median(stage) for stage in profile["stages"]]
                            for name,profile in (serial_idle or {}).get("profiles",{}).items()}
        self.samples = defaultdict(list)
        self.condition_samples = defaultdict(list)
        self.condition_rollback_samples = defaultdict(list)
        self.draft_samples = []
        ...
        for run in runs:
            ...
            cpu_samples[(run["k"],run["w"])].append(max(0.,run["elapsed_ms"]-service)/max(1,actions))
            ...
            for chunk in run["chunks"]:
                for stage, row in enumerate(chunk["stages"]):
                    ...
                    self.condition_samples[(run["k"],run["w"],stage,len(chunk["tokens"]))].append(row)
                self.draft_samples.extend(chunk["draft"])
                if chunk["rollback"]:
                    rb = chunk["rollback"]
                    for stage,row in enumerate(rb["commands"]):
                        self.rollback_samples[(stage,rb["accepted_replay_rows"])].append(row)
                        self.condition_rollback_samples[(run["k"],run["w"],stage,rb["accepted_replay_rows"])].append(row)
        ...
        self.condition_medians = {key:self.median(rows) for key,rows in self.condition_samples.items()}
        self.condition_rollback_medians = {key:self.median(rows) for key,rows in self.condition_rollback_samples.items()}
    ...
    def stage(self, chunk, stage, *, held_out=False, condition=None, network=None):
        # Periodic, non-speculative service has an observed idle dependence.
        # These are independent raw operation measurements, never a multiplier
        # fitted to validation wall time. Apply the same model during the sweep.
        if condition is not None and condition[0] == 0 and network in self.serial_idle:
            return self.serial_idle[network][stage], "independently_measured_periodic_serial_service"
        if not held_out and stage < len(chunk["stages"]):
            return chunk["stages"][stage], "exact_observed_operation"
        key = stage, len(chunk["tokens"])
        if condition is not None and (*condition,*key) in self.condition_medians:
            return self.condition_medians[(*condition,*key)], "empirical_same_K_W_stage_rows"
    ...
    def profile(self):
        ...
        for stage in range(3):
            ...
            prefill_rows = [dict(prompt_id=p["prompt_id"], k=p["k"], w=p["w"],
                                 compute_ms=sum(batch[stage]["compute_ms"] for batch in p["target"]),
                                 service_ms=sum(batch[stage]["service_ms"] for batch in p["target"])) for p in self.prefill]
            stats = self.state_stats[stage::3]
            state_times = {name: sum(r.get(name+"_ns",0) for r in stats)/max(1,sum(r.get(name+"_count",0) for r in stats))/1e6 for name in ["checkpoint","commit","rollback"]}
            stages.append(dict(stage="ABC"[stage], verification_by_rows=by_rows,
                               prefill_time_ms=float(np.median([r["compute_ms"] for r in prefill_rows])),
                               prefill_by_prompt_and_condition=prefill_rows,
                               deserialization_time_by_rows_ms={n:v["median"].get("deserialize_ms") for n,v in by_rows.items()},
                               peak_vram_bytes=None,
                               peak_vram_unavailable_reason="Per-stage device allocation peaks unavailable; sampled physical-device peak is recorded at profile level",

                               verification_time_by_K_ms={str(k):by_rows[str(k+1)]["median"]["compute_ms"] for k in [1,2,3]},
                               activation_input_bytes_by_rows={n:v["median"]["input_bytes"] for n,v in by_rows.items()},
                               activation_output_bytes_by_rows={n:v["median"]["output_bytes"] for n,v in by_rows.items()},
                               state_checkpoint_time_ms=state_times["checkpoint"],
                               state_commit_time_ms=state_times["commit"], state_rollback_time_ms=state_times["rollback"],
        ...
        return dict(experiment="E028_LOCAL_ASYNC_WAN_PROOF", evidence_class="PHYSICAL", stages=stages,
                    state_operation_totals=self.state_stats,serial_idle_calibration=self.serial_idle_source,
                    draft_time_per_token_ms=float(np.median([r["service_ms"] for r in self.draft_samples])),
                    prefill_measurements=self.prefill,
                    peak_vram_bytes=max((r.get("peak_vram_bytes") or 0 for r in self.runs),default=0),
                    cpu_scheduler_overhead_ms_per_action=[dict(k=k,w=w,median_ms=v) for (k,w),v in sorted(self.cpu_medians.items())],
                    speculative_acceptance_positions=[dict(prompt_id=r.get("prompt_id"),k=r["k"],w=r["w"],
                        accepted_prefix_lengths=[c["accepted"] for c in r["chunks"] if c["accepted"] is not None]) for r in self.runs if r["k"]],
...
class Replay:
    def __init__(self, run, measurements: Measurements, network, seed, *, held_out=False):
        ...
        self.initial_position = self.chunks[0]["position"]
        ...
        if held_out:
            self.cpu_per_action = measurements.cpu_medians[(run["k"],run["w"])]
        self.steady_end = 0.
    ...
    def stage_ready(self,index):
        ...
        row, provenance = self.m.stage(chunk,index,held_out=self.held_out,condition=(self.run["k"],self.run["w"]),network=self.net["name"])
    ...
    def decide(self):
        ...
        def commit():
            ...
            if head["position"] != self.initial_position+self.committed:
                raise AssertionError("commit violates causal sequence position")
            self.committed_stream.extend(head["tokens"][:accepted])
    ...
    def rollback(self, head):
        ...
        # All stages restore concurrently after receiving their control message.
        # Each native worker replays from its own bounded cached activation input.
        # The ACK carries compact position/checksum metadata, never context state.
        remaining = 3
        def restore(index):
            def received():
                self.stage_epoch[index] = head["epoch"]+1
                self.stage_pending[index].clear()
                row = self.m.condition_rollback_medians[(self.run["k"],self.run["w"],index,rb["accepted_replay_rows"])] if self.held_out else rb["commands"][index]
                def restored():
                    self.stage_position[index] = head["position"]+rb["accepted_replay_rows"]
                    self.stage_active[index] = False
                    self.transfer("ABC"[index],"coordinator",row["response_bytes"],acknowledged,kind="rollback_ack")
                self.resource("ABC"[index],row["service_ms"],restored,compute=row["compute_ms"],kind="rollback")
    ...
    def execute(self, *, include_events=False):
        ...
        if self.committed_stream != self.run["committed_tokens"]:
            raise AssertionError("simulation changed committed token stream")
        if self.committed != len(self.run["committed_tokens"]):
        ...
        row = dict(experiment="E028_LOCAL_ASYNC_WAN_PROOF", simulator_version=VERSION,
                   ...
                   **{f"stage_{name.lower()}_utilization":stage_util[name] for name in "ABC"},
                   steady_state_stage_utilization=steady_util, steady_interval_ms=steady_end-steady_start,
                   ...
                   proposed_speculative_positions=proposed,accepted_speculative_positions=accepted_draft,
                   ...
                   idle_service_profile_applied=self.run["k"]==0 and self.net["name"] in self.m.serial_idle,
                   stage_duration_source="independent periodic native SERIAL services for this network profile" if self.run["k"]==0 and self.net["name"] in self.m.serial_idle else "held-out empirical medians" if self.held_out else "exact physical chunk operation; empirical same-stage/same-row fallback for canceled work",
...
def load_serial_idle():
    path=OUT/"serial_idle_profile.json"
...
def load_workload():
    ...
    runs = []
    gpu_path = OUT/"gpu_samples.json"
    gpu_samples = json.loads(gpu_path.read_text()) if gpu_path.exists() else []
    for p in prompts:
        for k,w in [(0,1)]+[(k,w) for k in config["draft_lengths"] for w in config["inflight_windows"]]:
            ...
            if run.get("protocol_version") != "compound-cached-rollback-v2":
                raise ValueError(f"incompatible trace version: {path}")
            start = run["started_unix_seconds"]
            samples = [s["memory_bytes"] for s in gpu_samples if "memory_bytes" in s and start <= s["unix_seconds"] <= start+run["total_ms"]/1000]
            run["collection_peak_vram_so_far_bytes"] = run.get("peak_vram_bytes")
            run["peak_vram_bytes"] = max(samples) if samples else None
            run["peak_vram_sample_count"] = len(samples)
            runs.append(run)
    return config, runs
...
def aggregate(rows):
    ...
    for (network,k,w),rs in groups.items():
        ...
        row["speculative_acceptance_rate"] = sum(r["accepted_speculative_positions"] for r in rs)/max(1,sum(r["proposed_speculative_positions"] for r in rs))
        row["committed_tokens_per_target_traversal"] = tokens/max(1,sum(r["target_traversals"] for r in rs))
        ...
        row["steady_state_stage_utilization"] = {s:sum(r["steady_state_stage_utilization"][s]*r["steady_interval_ms"] for r in rs)/max(1e-9,sum(r["steady_interval_ms"] for r in rs)) for s in "ABC"}
        for metric in ["committed_tokens_per_target_traversal","speculative_acceptance_rate","ttft_ms","inter_token_latency_p50_ms","inter_token_latency_p95_ms"]:
            row["mean_run_"+metric] = sum(r[metric] for r in rs)/len(rs)
...
def sweep():
    ...
    if not validation["complete"] or not validation["passed"]:
        raise RuntimeError("Async projections are not qualified: the serial simulator gate must pass first")
    config,runs = load_workload()
    measurements = Measurements(runs,serial_idle=load_serial_idle())
    ...
    per_run_lookup = {(r["network"],r["prompt_id"],r["k"],r["w"],r["seed"]):r for r in rows}
    best_zero = max((a for a in aggs if a["network"]=="LOCAL" and a["k"]),key=lambda a:a["committed_tokens_per_second"])
    for r in rows:
        a=lookup[(r["network"],r["k"],r["w"])]
        if r["k"]:
            control = per_run_lookup[(r["network"],r["prompt_id"],r["k"],1,r["seed"])]
            zero = per_run_lookup[("LOCAL",r["prompt_id"],best_zero["k"],best_zero["w"],r["seed"])]
            r["async_speedup_vs_spec_sync"] = r["committed_tokens_per_second"]/control["committed_tokens_per_second"]
            r["throughput_retained_vs_zero_wan"] = r["committed_tokens_per_second"]/zero["committed_tokens_per_second"]
    ...
    best = max((r for r in aggs if r["network"]=="WAN-60" and r["w"]>1),key=lambda r:r["committed_tokens_per_second"])
    net = next(n for n in config["network_profiles"] if n["name"]=="WAN-60")
    for window in sorted({1,8,best["w"]}):
        source = next(r for r in runs if r["prompt_id"]==runs[0]["prompt_id"] and r["k"]==best["k"] and r["w"]==window)
        detail = Replay(source,measurements,net,config["network_seeds"][0]).execute(include_events=True)
        detail["physical_source_trace"] = f"traces/{source['prompt_id']}-k{source['k']}-w{window}.json"
        write_json(OUT/"traces"/f"virtual-wan60-k{best['k']}-w{window}.json",detail)
    return aggs
