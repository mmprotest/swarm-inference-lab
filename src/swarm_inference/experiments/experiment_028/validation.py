...
from .simulator import Measurements, Replay, load_workload, load_serial_idle, VERSION


async def validate(config, prompts, ranges):
    _, training_runs = load_workload()
    measured = Measurements(training_runs,serial_idle=load_serial_idle())
    ...
    with Workers(config,ranges,tag="validation") as workers:
        ...
        for k,rtt in cases:
            ...
            row=dict(k=k,w=1,network=net["name"],committed_tokens=len(run["committed_tokens"]),
                     ...
                     timing_source="independent native service: unshaped workload for speculation; separate-prompt periodic calibration for SERIAL; no wall-time fitting",
