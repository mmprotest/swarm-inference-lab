...
class CostModel:
    """Measured service model used only for tree selection.
    ...
    """
    def __init__(self,profile,vram_budget_bytes=32*2**30):
        sample=profile['samples'][0];count=len(sample['nodes'])
        self.stage_count=len(sample['stages'])
        self.hidden_bytes=(sample['stages'][1]['input_bytes']-8*count)//count
        self.max_feature_taps=max(len(stage['taps']) for stage in sample['stages'])
        self.max_profiled_nodes=max(row['node_budget'] for row in profile['samples'])
        self.max_profiled_depth=max(row['max_depth'] for row in profile['samples'])
        if profile['peak_vram_bytes']>vram_budget_bytes:
            raise MemoryError('Measured tree profile exceeds the configured VRAM budget')
    ...
    def estimate(self,nodes,net):
        ...
        bw=net['bandwidth_mbps'];network=(self.stage_count+3)/2*net['rtt_ms']
        if bw:
            network+=(len(nodes)*((self.stage_count-1)*self.hidden_bytes+8*self.stage_count+8)+expected*(self.max_feature_taps*self.hidden_bytes)+8*144)*8/(bw*1000)
...
def construct(policy,drafter:Drafter,budget,depth,network,cost:CostModel|None=None):
    if cost and (budget>cost.max_profiled_nodes or depth>cost.max_profiled_depth):
        raise ValueError('Tree bounds exceed the measured profile; profile that bound first')
