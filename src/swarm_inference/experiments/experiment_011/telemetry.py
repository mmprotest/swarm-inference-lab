"""Structured monotonic telemetry and dependency-based critical-path analysis."""
...
def reconstruct_critical_path(
    ...
    """Reconstruct serial dependencies from linked receive/compute trace events.

    A receive counts only when it is marked as token-critical and its
    ``unblocks_event_id`` resolves to a later required compute event.  Message
    totals alone are never used as the serial-wait count.
    """
