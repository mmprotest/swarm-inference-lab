...
class Workers:
    ...
    def __enter__(self):
        try:
            ...
            self.taps=self.draft.stats()['target_layers'] if self.draft else []
...
class Engine:
    ...
    def propose(self):
        ...
        meta,payload,trace=self.w.draft.call(15,position=self.position,n_tokens=8,arg=int(self.next_token))
        return np.frombuffer(payload,dtype='<f4').reshape(8,meta['width']).copy(),meta,trace
