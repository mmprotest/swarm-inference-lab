"""Execute only E029; resume completed prompt/configuration runs atomically."""
...
def main():
    ...
    with Workers(tag='workload') as w:
        ...
        write_json(OUT/'drafter_profile.json',dict(draft_backend='llama.cpp native DFlash2 scored-lattice adapter',draft_model=config['draft_model_path'],download=json.loads((ROOT/config['draft_model_path']).with_suffix('.download.json').read_text()),
            ...
            target_feature_layers=w.taps,target_feature_bytes_per_committed_token=len(w.taps)*5120*4,
            ...
            dflash2_used=True,fallback_used=False,block_size=8,selector_top_k=16,probability_transform='conditional selector softmax, temperature 1; not calibrated target acceptance probabilities'))
