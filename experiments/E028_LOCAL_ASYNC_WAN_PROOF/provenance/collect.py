...
async def stress(config, prompts, ranges):
    ...
    with GPURecorder() as gpu, Workers(config, ranges, tag="stress") as workers:
        ...
        for position, window in config["rollback_stress_scenarios"]:
            ...
            force = dict(position=position, remaining=target_events, reference=reference, n_vocab=248320)
            if path.exists():
                run = json.loads(path.read_text())
            else:
            ...
            result = dict(evidence_class="PHYSICAL", complete=len(rows)==len(config["rollback_stress_scenarios"]),
                          ...
                          passed=all(r["exact_continuation"] and r["forced_positions_correct"] and r["state_hash_checks_passed"] and not r["remaining_forced_events"] for r in rows), scenarios=rows)
