...
class LocalEngine:
    ...
    async def transfer_delay(self, payload_bytes):
        ...
        begin = self.now()
        # Windows asyncio timers rounded 15 ms requests to about 31 ms in the
        # saved E028 timer probe. Python's high-resolution sleep runs off-loop
        # so independent links still overlap while honoring the requested RTT.
        await asyncio.to_thread(time.sleep, duration/1000)
    ...
    async def prefill(self, tokens, *, use_draft=True):
        ...
        draft_traces = []
        if self.draft is not None and use_draft:
    ...
    async def run(self, tokens, *, k, w, count=256, force=None, fingerprints=False):
        ...
        self.events = []
        self.shaping = []
        next_token, next_hidden, prefill = await self.prefill(tokens, use_draft=bool(k))
        ...
        async def worker(index):
            while True:
                ...
                try:
                    result, tr = await self.call(self.stages[index], data, position=chunk.position, checkpoint=bool(k))
            ...
            while len(generated) < count:
                    ...
                    for j in range(block_size):
                        ...
                        if force is not None and not inflight and force["remaining"] > 0:
                            ...
                            if j <= force["position"]:
                                ...
                                if j == force["position"]:
                                    tok = 1 if tok == 0 else 0  # deterministic different valid token
                        block.append(tok)
                        if k:
                ...
                if head.rejected:
                    ...
                    for q in queues:
                        await q.join()
                    rb_begin = self.now()
                    async def restore(index, stage):
                        await self.transfer_delay(REQUEST.size)
                        async with self.gpu:
                            r = await asyncio.to_thread(stage._exchange, 9,
                                position=head.position, arg=accepted,
                                flags=Flags((1 << 10) if head.position in root_hashes else 0))
                        result = json.loads(r.payload)
                        await self.transfer_delay(r.wire_bytes)
                        if result["position"] != head.position+accepted:
                            raise AssertionError("rollback restored wrong sequence position")
                        prefix_equal = None
                        if fingerprints:
                            if index < 2:
                                payload = np.asarray(head.inputs[index+1][:accepted], dtype="<f4").tobytes()
                            else:
                                payload = out.top_ids[:accepted].astype("<i4").tobytes()
                                payload += out.top_logits[:accepted].astype("<f4").tobytes()
                                payload += out.nextn[:accepted].astype("<f4").tobytes()
                            hash_value = 14695981039346656037
                            for byte in payload:
                                hash_value = ((hash_value ^ byte) * 1099511628211) & ((1 << 64)-1)
                            prefix_equal = str(hash_value) == result["replay_output_fnv64"]
                            if not prefix_equal:
                                raise AssertionError("stage-local replay changed output bytes")
                        return response_trace(r, REQUEST.size), result, prefix_equal
                    restored = await asyncio.gather(*(restore(i,stage) for i,stage in enumerate(self.stages)))
                    commands = [r[0] for r in restored]
                    after = [r[1]["boundary_state"] for r in restored]
                    integrity = after == root_hashes[head.position] if head.position in root_hashes else None
                    rb = dict(chunk=head.id, boundary_position=head.position,
                              ...
                              commands=commands, replay=[], cached_stage_local_replay=True,
                              replay_output_checks=[r[2] for r in restored], state_hash_equal=integrity,
                              elapsed_ms=self.now()-rb_begin,
                              before_hashes=root_hashes.get(head.position), after_hashes=after)
                    head.rollback = rb
        ...
        return dict(k=k, w=w, committed_tokens=generated, token_sha256=token_hash(generated),
                    ...
                    protocol_version="compound-cached-rollback-v2", chunks=[c.record() for c in chunks], events=self.events,
