"""Retain H014-038q exact final-stage serving-phase thread attribution."""
...
from swarm_inference.protocol.stage_ring import StageMessage
...
def _gpu_health() -> dict[str, Any]:
    ...
    return {"status": "MEASURED", **dict(zip(fields, values, strict=True))}


async def _run(args: argparse.Namespace) -> dict[str, Any]:
    phase_records: list[dict[str, Any]] = []

    async def observe(phase: str, message: StageMessage) -> None:
        before = _snapshot()
    ...
    benchmark = await _benchmark_persistent_final_stage(
        ...
        cycle_id="H014-038q",
        execution_phase_observer=observe,
    )
    ...
    gates = {
        "benchmark_pass": benchmark["status"] == "PASS",
        ...
        is True,
        "all_expected_phases_observed": (
            len(phase_records) == 12
            and {row["phase"] for row in phase_records}
            == {"input_unpacked", "compute_complete", "response_built"}
        ),
        "gpu_identity_stable": (
    ...
    return {
        "schema_version": "experiment-014-h014-038q-serving-phase-thread-v1",
        "cycle_id": "H014-038q",
        "status": "PASS" if all(gates.values()) else "FAIL",
        "hypothesis": (
            "Real final-stage CUDA execution/completion, rather than input unpack or "
            "token-result packing, asynchronously creates the remaining singleton."
        ),
        "hypothesis_reproduced": (
            trigger_phase == "compute_complete" and trigger_is_non_python_singleton
        ),
