    ...
    for key in ["model_path", "draft_model_path"]:
        ...
        with p.open("rb") as f:
            for b in iter(lambda: f.read(16*1024*1024), b""):
                h.update(b)
        expected = config["model_sha256" if key == "model_path" else "draft_model_sha256"]
        if h.hexdigest() != expected:
            raise ValueError("local model hash does not match the registered artifact")
...
def main():
    ...
    parser.add_argument("phase", choices=["prepare", "profile", "smoke", "collect", "stress", "validate", "sweep", "report"])
    ...
    if args.phase == "prepare":
        ...
        from swarm_inference.experiments.experiment_028.report import finish
        finish()
