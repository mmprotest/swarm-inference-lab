...
def main():
    ...
    with Workers(tag='profile') as w:
        ...
        # Measured node-budget/depth scaling, fixed shapes and prompts.
        for g in (gs[0],gs[-1]):
            ...
            for N in (8,16,32):
                for D in (4,6,8):
                    for rep in range(2):
                        ...
                        write_json(OUT/'tree_verification_profile.json',dict(samples=samples,peak_vram_bytes=peak,cuda_execution_time_ms=None,cuda_note='Host llama_decode + device synchronization timing; isolated CUDA event duration unavailable.',branch_memory='canonical KV sequence and device partial-state snapshots per active depth; no full-context copy per node',verification='scalar DFS nodes, state restores and accepted-path replay',optional_64='not attempted; required node budgets are 8,16,32'))
        ...
        write_json(OUT/'tree_verification_profile.json',dict(samples=samples,peak_vram_bytes=max(peak,w.vram()),cuda_execution_time_ms=None,verification='Canonical DFS with device partial-state checkpoints and KV-tail truncation; accepted path replay charged; linear-chain checkpoint optimization',cuda_note='Host llama_decode + synchronization, not CUDA-event-only measurement',optional_64='not attempted; required node budgets are 8,16,32'))
