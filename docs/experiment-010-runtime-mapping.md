...
| `colibri_expert_bank.py` | RETIRED | The historical exact-byte container was removed. Canonical routed-expert inventory, loading, and bounded residency are owned by `swarm_inference.backends.colibri.torch_backend.ColibriMoeBackend`. | Archived evidence only; no import path remains. | None; extend the architecture-neutral canonical backend only. | Already deleted. |
